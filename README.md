...
При отправке некорректных данных API возвращает стандартные сообщения об ошибках валидации Laravel:
...
```json
{
  ...
  "errors": {
    ...
    "status": [
      "Статус должен быть одним из следующих: pending, in_progress, completed"
    ]
  }
}
```
...
1. **Form Requests** - для валидации используются отдельные классы запросов:
  - `CreateTaskRequest` - для создания задач
  - `UpdateTaskRequest` - для обновления задач

2. **API Resources** - для форматирования ответов используются ресурсы:
  - `TaskResource` - для форматирования данных задачи

3. **Пагинация** - список задач выводится с пагинацией по 10 записей на страницу
